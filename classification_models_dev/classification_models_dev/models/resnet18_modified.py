...
def handle_block_names(stage, block, encoder_activation='relu'):
    name_base = 'stage{}_unit{}_'.format(stage + 1, block + 1)
    conv_name = name_base + 'conv'
    bn_name = name_base + 'bn'
    # relu_name = name_base + 'relu'
    activation_name = name_base + encoder_activation
    sc_name = name_base + 'sc'
    # return conv_name, bn_name, relu_name, sc_name
    return conv_name, bn_name, activation_name, sc_name
...
def residual_conv_block(filters, stage, block, strides=(1, 1), attention=None, cut='pre', encoder_activation='relu'):
    print("encoder_activation: ", encoder_activation)
...
def ResNet_Modified(model_params, input_shape=None, input_tensor=None, include_top=True, encoder_activation='relu',
    ...
    print('[DEBUG] constructing resnet')
        ...
        for block in range(rep):
            ...
            if block == 0 and stage == 0:
                x = ResidualBlock(filters, stage, block, strides=(1, 1),
                                  cut='post', encoder_activation=encoder_activation, attention=Attention)(x)
                ...
                x = ResidualBlock(filters, stage, block, strides=(1, 1),
                                  cut='pre', encoder_activation=encoder_activation, attention=Attention)(x)
...
MODELS_PARAMS = {
    'resnet18_modified': ModelParams('resnet18_modified', (2, 2, 2, 2), residual_conv_block, None),
}


def ResNet18_Modified(input_shape=None, input_tensor=None, weights=None, classes=1000, include_top=True, encoder_activation='relu', **kwargs):
    print("using activation ", encoder_activation)
    return ResNet_Modified(
        MODELS_PARAMS['resnet18_modified'],
...
setattr(ResNet18_Modified, '__doc__', ResNet_Modified.__doc__)